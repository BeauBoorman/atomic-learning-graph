...
| We caught a model failure the deterministic gates structurally cannot see | `data/graph.json` is written only by `pnpm atomize`. Its sha256 is pinned in `data/graph.run.json`, and `src/atomization/graph-run.test.ts` recomputes it — so an UNDISCLOSED hand-edit turns the suite red. There is exactly **one disclosed human correction on record**: a human caught that the model's self-attention worked example demonstrated *uniform averaging* (which is average pooling, not attention). That is the class of error the deterministic citation gates cannot structurally see — they verify that the prose's `quotedText` bytes occur verbatim in the source, not that the prose's *arithmetic* demonstrates the *concept it claims to*. The correction touched lesson prose and its analogies only; every `quotedText` citation, every source byte, every concept id, and every structural invariant is unchanged. It is documented in [`PROVENANCE-PATCHES.md`](PROVENANCE-PATCHES.md) and under `humanPatches` in [`data/graph.run.json`](data/graph.run.json), and the project's position is that a human-in-the-loop catch disclosed on the record is the honest answer to a gate-blind-spot — not a contradiction of the machine-emitted claim, but the strongest possible evidence the discipline behind it is real. |
