...
export interface CorpusVerificationOptions {
  repoRoot: string;
  verifyUpstream?: boolean;
}

/** Verify the committed, locally stored D2L corpus and its generated license artifacts. */
export async function verifyCorpus({
  repoRoot,
  verifyUpstream = false,
}: CorpusVerificationOptions): Promise<{ entryCount: number }> {
  const oerDir = resolve(repoRoot, "data", "oer");
  const manifestPath = resolve(oerDir, "sources.json");
  const raw = loadManifest(manifestPath);
  const validated = validateManifest(raw, oerDir);
  ...
  const unlistedFiles = readdirSync(oerDir).filter((name) => !allowedFiles.has(name));
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    const markdownPath = resolve(oerDir, localMarkdownPath(entry.textPath));
    const textPath = resolve(oerDir, entry.textPath);
  ...
  if (verifyUpstream) {
    for (const entry of entries) {
      const storedMarkdown = readFileSync(resolve(oerDir, localMarkdownPath(entry.textPath)));
